...
AtmoClient.prototype.getPortMappings = function (cb) {
    request = https.request({
        ...
        if (res.statusCode != 200) {
            cb(new Error('error getting port mappings:' + res.toString()));
            //console.log(res);
            return;
        }
...
AtmoClient.prototype.getPortMappingTemplate = function (portMappingTemplateId, cb) {
    request = https.request({
        ...
        //console.log(res.statusCode);
        if (res.statusCode != 200) {
            cb(new Error('error getting port mapping template:' + res.toString()));
            //console.log(res);
            return;
        }
        res.setEncoding('utf8');
...
AtmoClient.prototype.getVirtualMachine = function (id, cb) {
    request = https.request({
        ...
        //console.log(res.statusCode);
        if (res.statusCode != 200) {
            cb(new Error('error getting vm:' + res.toString()));
            //console.log(res);
            return;
        }
        res.setEncoding('utf8');
...
AtmoClient.prototype.newAppliance = function (appliance, cb) {
    ...
    request = https.request({
        ...
        if (res.statusCode != 201) {
            //console.log(res);
            cb(new Error('error creating appliance:' + res.toString()));
            return;
        }
        res.setEncoding('utf8');
        var data = '';
...
AtmoClient.prototype.getApplianceSets = function (cb) {
    var request = https.request({
        ...
        //console.log(res.statusCode);
        if (res.statusCode != 200) {
            cb(new Error('error getting appliance sets:' + res.toString()));
            //console.log(res);
            return;
        }
        res.setEncoding('utf8');
...
AtmoClient.prototype.deleteApplianceSet = function (applianceSetId, cb) {
    var request = https.request({
        ...
        //console.log(res.statusCode);
        if (res.statusCode != 200) {
            cb(new Error('error deleting appliance sets:' + res.toString()));
            //console.log(res);
            return;
        }
        cb(null);
